...
| **[tps.user.js](tps.user.js)** | Tek Pencere Sistemi (TPS) belge listesi sayfasında, arama kutusunun yanına yıl ve belge serisi seçimi için pratik bir dropdown menü ekler. Yıl seçenekleri en yeni en üstte olacak şekilde sıralanır. | 1.52 |
| **[combinedonbildirim.user.js](combinedonbildirim.user.js)** | Önbildirim GGBS sayfaları için hepsi bir arada araç. Captcha otomatik doldurma, form alanı güncellemeleri, sayfa tipine göre akıllı buton gösterimi (Ürün/Sevkiyat sorgulama, Sertifika/İçerik/Etiket indirme), ID kopyalama ve form sıfırlama özellikleri. | 2.230 |
| **[konatfaturaislemleri.user.js](konatfaturaislemleri.user.js)** | Konat için fatura ve menü işlemlerini birleştiren hepsi bir arada çözüm. Özellikler: PDF indirme/birleştirme, tarih/firma filtresi, satır gizleme/renklendiring, otomatik onay, menü kısayolları. | 4.2 |
...
| **[mobilggbsonbildirim.user.js](mobilggbsonbildirim.user.js)** | Mobil görünümde gizlenen Gümrük Başvuru No sütununu görünür yapar, captcha alanını otomatik doldurur ve Ön Bildirim No yanına, tıklandığında ID'yi panoya kopyalayan şık bir "ID" butonu ekler. Ayrıca Ön Bildirim yazdırma sayfasını (`OnBildirimDosyaIndir`) A4'e sığacak şekilde otomatik ölçekler (dinamik zoom). | 4 |
...
* **27.08.2026:** `mobilggbsonbildirim.user.js` güncellendi (v4): Ön Bildirim yazdırma sayfasına (`OnBildirimDosyaIndir`) A4 düzeni ve yazdırmadan önce içeriği A4'e tam sığdıracak dinamik otomatik zoom eklendi. `@run-at document-start` ile başlatma iyileştirildi, kod iki bölüme ayrıldı (sütun/captcha araçları + A4 yazdırma).
* **27.08.2026:** `tps.user.js` güncellendi (v1.52): Yıl seçenekleri yeniden sıralandı; en güncel yıl artık en üstte ve doğru değerine bağlı. Sürüm 1.5'ten 1.52'ye yükseltildi.
* **03.06.2026:** `combinedonbildirim.user.js` güncellendi (v2.230): Sertifika İndir butonunun aktif/mevcut olduğundaki arka plan rengi, hata veya silme eylemini çağrıştıran yanıltıcı kırmızı renkten (`#f44336`) güven veren mavi renge (`#1e88e5`) çekildi.
* **03.06.2026:** `combinedonbildirim.user.js` güncellendi (v2.220): Tıklanan kutucukların (butonlar ve girdi alanları) odaklanma ve tıklama renklerini uyumlu hale getiren CSS geçişleri, odaklanma halkaları (focus outlines) ve kopyalama işlemi sonrasında yeşil onay bildirimi efekti (active flash) eklendi.
* **02.06.2026:** `combinedonbildirim.user.js` güncellendi (v2.210): Buton paneline personel adı kısaltmasını (örn. "EE") kodlara müdahale etmeden sayfa üzerinden değiştirebilmeyi sağlayan kalıcı hafızalı (`localStorage`) ayar kutusu ve gerçek zamanlı doldurma desteği eklendi.
* **02.06.2026:** `combinedonbildirim.user.js` güncellendi (v2.200): Sertifika İndir, İçerik İndir ve Etiket İndir butonlarının yanına, ilgili belgelerin mevcut olup olmadığını gösteren görsel durum belirticileri (Var/Yok rozetleri) eklendi. Belge yoksa butonlar görsel olarak pasifleştirildi.
* **20.05.2026:** `mobilggbsonbildirim.user.js` güncellendi (v3.0): `mobil2ggbsonbildirim.user.js` (captcha otomatik doldurma) betiği ile birleştirildi. Artık hem captcha otomatik doldurma hem de ID kopyalama butonu tek bir script üzerinden çalışıyor.

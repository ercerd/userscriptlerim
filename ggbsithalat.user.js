...
(function() {
    ...
    // buttondokumanonayla için özel waitForElementWithTimeoutAndReject
    ...
    const findAndClickIthalat = () => {
        ...
        const proceedWithIthalatIframe = (targetIframeDoc) => {
            return new Promise((resolve, reject) => {
                let attempts = 0;
                        ...
                        if (inputField && !inputField.disabled) {
                            ...
                            navigator.clipboard.readText()
                                .then((text) => {
                                    ...
                                    if (onbildirimRegex.test(text)) {
                                        inputField.value = text;
                                        triggerEvent(inputField, 'change');
                                        triggerEvent(inputField, 'input');
                                        GM_log('F_32 input alanına panodaki metin yapıştırıldı: ' + text);
                                        resolve(obsIframeDoc);
                                    } else {
                                        const text = prompt('Panodaki Metin Önbildirim sayısı değil veya yapıştırılamadı. Önbildirim numarasını giriniz (örn. 20-110236-300):');
                                        if (text) {
                                            inputField.value = text;
                                            triggerEvent(inputField, 'change');
                                            triggerEvent(inputField, 'input');
                                            GM_log('F_32 input alanına kullanıcı metni yapıştırıldı: ' + text);
                                            resolve(obsIframeDoc);
                                        } else {
                                            GM_log('Kullanıcı metin girmedi');
                                            throw new Error('Metin girilmedi');
                                        }
                                    }
                                })
                                .catch((error) => {
                                    GM_log('Panodan metin okuma hatası: ' + error);
                                    const text = prompt('Panodaki Metin Önbildirim sayısı değil veya yapıştırılamadı. Önbildirim numarasını giriniz (örn. 20-110236-300):');
                                    if (text) {
                                        inputField.value = text;
                                        triggerEvent(inputField, 'change');
            ...
            .catch(error => {
                GM_log('Error in proceedWithIthalatIframe: ' + error);
            });
        };
        ...
        .then((targetIframeDoc) => proceedWithIthalatIframe(targetIframeDoc))
        .catch(error => {
            GM_log('Error in findAndClickIthalat: ' + error);
        });
    };
